...
class observation_before(TypedDict, total=False):
    screen_summary: str
    dom_summary: str
...
def make_test_plan_node(state: PlaytestState) -> PlaytestState:
    ...
    try:
        ...
        # Check if we are retrying after a validation failure
        validation_error = state.get("error_message")
        feedback_prompt = ""
        if state.get("if_valid_test_plan") is False and validation_error:
            feedback_prompt = (
                f"\n\nCRITICAL: A previous test plan was rejected during validation with the following error:\n"
                f"\"{validation_error}\"\n"
                f"Please generate a new, improved test plan that addresses this feedback directly."
            )
        ...
        prompt_text = (
            ...
            f"3. Key checkpoints/states to verify during execution to ensure the game works properly.{feedback_prompt}"
        )
...
class TestPlanValidation(BaseModel):
    if_valid_test_plan: bool = Field(description="True if the test plan is valid, detailed, covers the main game checkpoints, and is executable. False otherwise.")
    error_message: str = Field(description="Explanation of why the plan is invalid, or empty string if it is valid.")

def validate_test_plan_node(state: PlaytestState) -> PlaytestState:
    """Validates the generated test plan using Gemini."""
    try:
        test_plan = state.get("test_plan", [])
        plan_reasoning = state.get("plan_reasoning", "")
        critical_checkpoints = state.get("critical_checkpoints", [])
        
        prompt_text = (
            "You are a Senior QA Manager reviewing a test plan generated by a playtesting assistant.\n"
            "Review the test plan steps, reasoning, and critical checkpoints for completeness, logical flow, and feasibility.\n\n"
            f"Test Plan Steps:\n"
            + "\n".join(f"- {step}" for step in test_plan) + "\n\n"
            f"Plan Reasoning:\n{plan_reasoning}\n\n"
            f"Critical Checkpoints:\n"
            + "\n".join(f"- {cp}" for cp in critical_checkpoints) + "\n\n"
            "If the plan is valid, set if_valid_test_plan to True and error_message to an empty string.\n"
            "If the plan is invalid, set if_valid_test_plan to False and describe the shortcomings in error_message."
        )
        
        message = HumanMessage(content=prompt_text)
        structured_llm = model_1.with_structured_output(TestPlanValidation)
        validation = structured_llm.invoke([message])
        
        print("\n[Node Executed] validate_test_plan_node")
        print(f"Validation Result: {validation.if_valid_test_plan}")
        
        if not validation.if_valid_test_plan:
            print(f"Validation Error: {validation.error_message}")
            updated_plan = list(test_plan) + [f"Validation Error: {validation.error_message}"]
            return {
                "if_valid_test_plan": False,
                "error_message": validation.error_message,
                "test_plan": updated_plan
            }
        
        return {
            "if_valid_test_plan": True,
            "error_message": ""
        }
    except Exception as e:
        print(f"Error in validate_test_plan_node: {e}")
        return {
            "if_valid_test_plan": False,
            "error_message": f"failed_to_validate_test_plan: {e}"
        }

def observe_before_node(state: PlaytestState) -> PlaytestState:
    """Placeholder node for observe_before step."""
    print("\n[Node Executed] observe_before_node")
    return {}

def route_test_plan_ready(state: PlaytestState) -> str:
    """Routes the graph based on test plan validation."""
    if_valid = state.get("if_valid_test_plan", False)
    if if_valid:
        print("Test plan is valid. Routing to observe_before...")
        return "observe_before"
    else:
        print("Test plan is invalid. Routing back to make_test_plan...")
...
graph.add_node("validate_test_plan", validate_test_plan_node)
...
# Go to validate_test_plan after making the test plan
graph.add_edge("make_test_plan", "validate_test_plan")

# Route conditionally based on test plan validation
graph.add_conditional_edges(
    "validate_test_plan",
...
# Route observe_before to END
graph.add_edge("observe_before", END)
